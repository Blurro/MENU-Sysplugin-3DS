...
static Result loadCode(const ExHeader_Info *exhi, u64 programHandle, const prog_addrs_t *mapped)
{
    ...
    patchCode(titleId, csi->flags.remaster_version, (u8 *)mapped->text_addr, mapped->total_size << 12, csi->text.size, csi->rodata.size, csi->data.size, csi->rodata.address, csi->data.address); // PLG_MARKER(menu_loader_patch_code_call)
...
static Result LoadProcessImpl(Handle *outProcessHandle, const ExHeader_Info *exhi, u64 programHandle)
{
    ...
    if (R_SUCCEEDED(res = loadCode(exhi, programHandle, &mapped)))
    {
        ...
        res = svcCreateCodeSet(&codeset, &csh, mapped.text_addr, mapped.ro_addr, mapped.data_addr); // PLG_MARKER(menu_loader_createcodeset_call)
        if (R_SUCCEEDED(res))
        {
            // There are always 28 descriptors
            res = svcCreateProcess(outProcessHandle, codeset, exhi->aci.kernel_caps.descriptors, count); // PLG_MARKER(menu_loader_createprocess_call)
            ...
            if (!res && ((u32)((titleId >> 0x20) & 0xFFFFFFEDULL) == 0x00040000))
            {
                ...
                assertSuccess(plgldrInit()); // PLG_MARKER(menu_loader_plgldr_init_call)
                assertSuccess(PLGLDR_LoadPlugin(processID, isHomebrew));
                plgldrExit(); // PLG_MARKER(menu_loader_plgldr_exit_call)
            }
        }
    }
