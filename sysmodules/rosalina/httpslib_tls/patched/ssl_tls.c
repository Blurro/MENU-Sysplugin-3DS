...
#if defined(MENU_TLS_MIN_CERT)
static int menu_tls_skip_tag(unsigned char **p,
...
static int menu_tls_find_spki(unsigned char *crt,
                              size_t crt_len,
                              unsigned char **spki,
                              size_t *spki_len)
{
    ...
    size_t len;
    unsigned char *p = crt;
    const unsigned char *end = crt + crt_len;
    const unsigned char *crt_end;
    const unsigned char *tbs_end;
    unsigned char *spki_start;
    unsigned char *q;
    ...
    ret = mbedtls_asn1_get_tag(&p, crt_end, &len,
                               MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE);
    if (ret != 0 || len > (size_t)(crt_end - p)) {
        return MBEDTLS_ERR_SSL_BAD_HS_CERTIFICATE;
    }
    tbs_end = p + len;

    if (p < tbs_end && *p == (MBEDTLS_ASN1_CONTEXT_SPECIFIC |
                               MBEDTLS_ASN1_CONSTRUCTED | 0)) {
        if (menu_tls_skip_tag(&p, tbs_end,
    ...
    if (menu_tls_skip_tag(&p, tbs_end, MBEDTLS_ASN1_INTEGER) != 0 ||
        menu_tls_skip_tag(&p, tbs_end,
    ...
    spki_start = p;
    q = p;
    ret = mbedtls_asn1_get_tag(&q, tbs_end, &len,
                               MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE);
    if (ret != 0 || len > (size_t)(tbs_end - q)) {
        return MBEDTLS_ERR_SSL_BAD_HS_CERTIFICATE;
    }

    *spki = spki_start;
    *spki_len = (size_t)((q + len) - spki_start);
    return 0;
}
...
int mbedtls_ssl_parse_certificate(mbedtls_ssl_context *ssl)
{
    ...
    size_t i, chain_len, crt_len, spki_len;
    unsigned char *spki;
    unsigned char *pk_pos;
    size_t hs_hdr_len;
    ...
    hs_hdr_len = mbedtls_ssl_hs_hdr_len(ssl);
    if (ssl->in_msgtype != MBEDTLS_SSL_MSG_HANDSHAKE ||
        ssl->in_msg[0] != MBEDTLS_SSL_HS_CERTIFICATE ||
        ssl->in_hslen < hs_hdr_len + 6) {
        ret = MBEDTLS_ERR_SSL_BAD_HS_CERTIFICATE;
        goto exit;
    }

    i = hs_hdr_len;
    chain_len = ((size_t)ssl->in_msg[i] << 16) |
                ((size_t)ssl->in_msg[i + 1] << 8) |
                (size_t)ssl->in_msg[i + 2];
    i += 3;
    if (chain_len != ssl->in_hslen - i || chain_len < 3) {
        ret = MBEDTLS_ERR_SSL_BAD_HS_CERTIFICATE;
        goto exit;
    }

    crt_len = ((size_t)ssl->in_msg[i] << 16) |
              ((size_t)ssl->in_msg[i + 1] << 8) |
              (size_t)ssl->in_msg[i + 2];
    i += 3;
    if (crt_len < 128 || crt_len > ssl->in_hslen - i) {
        ret = MBEDTLS_ERR_SSL_BAD_HS_CERTIFICATE;
        goto exit;
    }

    ret = menu_tls_find_spki(ssl->in_msg + i, crt_len, &spki, &spki_len);
    if (ret != 0) {
        goto exit;
    }

    mbedtls_pk_free(&ssl->handshake->peer_pubkey);
    mbedtls_pk_init(&ssl->handshake->peer_pubkey);
    pk_pos = spki;
    ret = mbedtls_pk_parse_subpubkey(&pk_pos, spki + spki_len,
                                     &ssl->handshake->peer_pubkey);
    if (ret != 0 || pk_pos != spki + spki_len) {
        ret = MBEDTLS_ERR_SSL_BAD_HS_CERTIFICATE;
        goto exit;
    }
...
int mbedtls_ssl_write_finished(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->handshake->calc_finished == NULL) {
        return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
    }
